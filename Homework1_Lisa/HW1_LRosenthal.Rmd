...
```{r load}
CO2 <- read.table("~/Desktop/Stat Model class F2017/Homework1/CO2_HW1.txt",header=T)
```
...
ec <- read.table("~/Desktop/Stat Model class F2017/Homework1/ecdata_HW1.txt",header=T)
dev.off()
```
...
curve(dgamma(x, shape=shape.start, scale=scale.start), from=10, to=60, col=5, add=T)
...
model.normal <- mle2(ffdate~dnorm(mean=mu, sd=sigma), data=ec, start=list(mu=mean(ec$ffdate), sigma=sd(ec$ffdate)))

#i have no clue. start up here again. 
model.gamma <- mle2(ffdate~dgamma(x, shape=shape.start, scale=scale.start), data=ec, start=list(mu=mean(ec$ffdate), sigma=sd(ec$ffdate)))
```
